...
static lv_res_t _action_flash_linux_data(lv_obj_t * btns, const char * txt)
{
	...
	lv_mbox_set_text(mbox, "#FF8000 刷入Linux#");
	...
	if (res)
	{
		lv_label_set_text(lbl_status, "#FFDD00 错误:# 打开分区1失败!");
	...
	while (total_size_sct > 0)
	{
		...
		if (bytesWritten >= fileSize)
		{
			...
			if (res)
			{
				s_printf(txt_buf, "#FFDD00 错误:# 打开分区%d失败#", currPartIdx);
		...
		while (res)
		{
			...
			if (retryCount >= 3)
			{
				lv_label_set_text(lbl_status, "#FFDD00 错误:# 向SD卡写入!");
				manual_system_maintenance(true);
...
static lv_res_t _action_check_flash_linux(lv_obj_t *btn)
{
	...
	lv_obj_t *lbl_status = lv_label_create(mbox, NULL);
	lv_label_set_recolor(lbl_status, true);
	lv_label_set_text(lbl_status, "#C7EA46 状态:# 正在搜索文件和分区...");
	...
	if (f_stat(path, NULL))
	{
		lv_label_set_text(lbl_status, "#FFDD00 错误:# 未找到安装文件!");
		goto error;
	}
	...
	{
		lv_label_set_text(lbl_status, "#FFDD00 错误:# 未找到分区!");
		goto error;
	}
	...
	{
		lv_label_set_text(lbl_status, "#FFDD00 错误:# 镜像大小比分区大!");
		goto error;
	}
	...
	s_printf(txt_buf,
		...
		"\n是否继续?", l4t_flash_ctxt.offset_sct, size_sct, l4t_flash_ctxt.image_size_sct >> 11);
	lv_label_set_text(lbl_status, txt_buf);
	free(txt_buf);
...
static lv_res_t _action_flash_android_data(lv_obj_t * btns, const char * txt)
{
	...
	lv_mbox_set_text(mbox, "#FF8000 刷入Android#");

	lv_obj_t *lbl_status = lv_label_create(mbox, NULL);
	lv_label_set_recolor(lbl_status, true);
	lv_label_set_text(lbl_status, "#C7EA46 状态:# 搜索文件和分区...");
	...
	if (f_stat(path, NULL))
	{
		s_printf(txt_buf, "#FF8000 警告:# 未找到内核镜像!\n");
	...
	if (offset_sct && size_sct)
	{
		...
		if ((file_size >> 9) > size_sct)
			s_printf(txt_buf, "#FF8000 警告:# 内核镜像过大!\n");
		else
		{
			sdmmc_storage_write(part_info.storage, offset_sct, file_size >> 9, buf);

			s_printf(txt_buf, "#C7EA46 成功:# 内核镜像已刷入!\n");
			f_unlink(path);
		}
		...
		s_printf(txt_buf, "#FF8000 警告:# 未找到内核分区!\n");
...
boot_img_not_found:
	...
	if (offset_sct && size_sct)
	{
		...
		if ((file_size >> 9) > size_sct)
			strcat(txt_buf, "#FF8000 警告:# Recovery镜像过大!\n");
		else
		{
			sdmmc_storage_write(part_info.storage, offset_sct, file_size >> 9, buf);
...
recovery_not_found:
	...
	if (offset_sct && size_sct)
	{
		...
		if ((file_size >> 9) > size_sct)
			strcat(txt_buf, "#FF8000 警告:# DTB镜像过大!");
		else
		{
			sdmmc_storage_write(part_info.storage, offset_sct, file_size >> 9, buf);
			strcat(txt_buf, "#C7EA46 成功:# DTB镜像已刷入!");
			f_unlink(path);
		}
...
static lv_res_t _sd_create_mbox_start_partitioning()
{
	...
	static const char *mbox_btn_map[] =  { "\251", "\222确定", "\251", "" };
	static const char *mbox_btn_map1[] = { "\222SD USB大容量存储", "\222刷入Linux", "\222刷入Android", "\221确定", "" };
	static const char *mbox_btn_map2[] = { "\222SD USB大容量存储", "\222刷入Linux", "\221确定", "" };
	static const char *mbox_btn_map3[] = { "\222SD USB大容量存储", "\222刷入Android", "\221确定", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox, "#FF8000 SD分区管理#");
	lv_obj_align(mbox, NULL, LV_ALIGN_CENTER, 0, 0);
	lv_obj_set_top(mbox, true);
	...
	lv_label_set_text(lbl_status, "#00DDFF 状态:# 初始化Ramdisk中...");
	lv_label_set_text(lbl_paths[0], "请等待...");
	lv_obj_align(mbox, NULL, LV_ALIGN_CENTER, 0, 0);
	manual_system_maintenance(true);
	...
	{
		lv_label_set_text(lbl_status, "#FFDD00 错误:# 初始化Ramdisk失败!");
		goto error;
	}

	lv_label_set_text(lbl_status, "#00DDFF 状态:# 备份文件中...");
	manual_system_maintenance(true);
	...
	lv_label_set_text(lbl_status, "#00DDFF 状态:# 格式化FAT32分区...");
	lv_label_set_text(lbl_paths[0], "请等待...");
	lv_label_set_text(lbl_paths[1], " ");
	manual_system_maintenance(true);
	...
	if (mkfs_error)
	{
		...
		lv_label_set_text(lbl_status, "#00DDFF 状态:# 恢复文件中...");
		manual_system_maintenance(true);
		...
		if (_backup_and_restore_files(false, lbl_paths))
		{
			...
			if (_backup_and_restore_files(false, lbl_paths))
			{
				lv_label_set_text(lbl_status, "#FFDD00 错误:# 恢复文件失败!");
				free(buf);
				goto error;
			}
		}

		lv_label_set_text(lbl_status, "#00DDFF 状态:# 文件已恢复但操作失败!");
		f_unmount("ram:");
		free(buf);
		goto error;
	}
...
mkfs_no_error:
	...
	lv_label_set_text(lbl_status, "#00DDFF 状态:# 刷入分区表...");
	lv_label_set_text(lbl_paths[0], "请等待...");
	lv_label_set_text(lbl_paths[1], " ");
	manual_system_maintenance(true);
...
static lv_res_t _emmc_create_mbox_start_partitioning()
{
	...
	static const char *mbox_btn_map[] =  { "\251", "\222确定", "\251", "" };
	static const char *mbox_btn_map1[] = { "\222刷入Linux", "\222刷入Android", "\221确定", "" };
	static const char *mbox_btn_map2[] = { "\222刷入Linux", "\221确定", "" };
	static const char *mbox_btn_map3[] = { "\222刷入Android", "\221确定", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox, "#FF8000 eMMC分区管理#");
	lv_obj_align(mbox, NULL, LV_ALIGN_CENTER, 0, 0);
	lv_obj_set_top(mbox, true);
	...
	// Use safety wait if backup is not possible.
	char *txt_buf = malloc(SZ_4K);
	strcpy(txt_buf, "#FF8000 eMMC分区管理#\n\n误操作保护, ");
	lv_mbox_set_text(mbox, txt_buf);
	...
	{
		lv_label_set_text(lbl_extra, "#FFDD00 格式化USER分区需要BIS密钥!#");
		emmc_end();
		goto exit;
	}
...
static lv_res_t _create_mbox_partitioning_warn()
{
	...
	static const char *mbox_btn_map[] = { "\222SD USB大容量存储", "\222开始", "\222取消", "" };
	static const char *mbox_btn_map1[] = { "\222开始", "\222取消", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);

	char *txt_buf = malloc(SZ_4K);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
	lv_mbox_set_text(mbox, "#FF8000 分区管理器#");
	...
	if (!part_info.emmc)
	{
		...
		if (part_info.backup_possible)
		{
			strcat(txt_buf, "#C7EA46 卡内的文件将自动备份及恢复!#\n"
				"#FFDD00 任何其他分区将擦除!#");
		}
		else
		{
			strcat(txt_buf, "#FFDD00 卡内的文件将被清除!#\n"
				"#FFDD00 任何其他分区也将擦除!#\n"
				"#FFDD00 请使用USB大容量存储将文件复制到其他地方!#");
		}
...
static lv_res_t _create_mbox_partitioning_andr_part()
{
	...
	static const char *mbox_btn_map[] = { "\222传统", "\222动态", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	...
	lv_label_set_text(lbl_status,
		"请选择一种分区模式:\n\n"
		"#C7EA46 动态:# Android 13+ (推荐)\n"
		"#C7EA46 传统:# Android 11\n");
...
static void _create_mbox_check_files_total_size()
{
	...
	static const char *mbox_btn_map[] = { "\251", "\222确定", "\251", "" };
	static const char *mbox_btn_map2[] = { "\222不备份", "\222确定", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
	...
	if (part_info.backup_possible)
	{
		s_printf(txt_buf,
			"#96FF00 SD卡文件可自动备份,#\n"
			"#FFDD00 任何其他分区都将被擦除!#\n"
			"#00DDFF 文件总数:# %d, #00DDFF 总大小:# %d MiB", total_files, total_size >> 20);
		lv_mbox_set_text(mbox, txt_buf);
	}
	else
	{
	...
	lv_label_set_text(lbl_part, "#00DDFF 当前MBR分区布局:#");
...
static lv_res_t _action_fix_mbr_gpt(lv_obj_t *btn)
{
	...
	{
		lv_label_set_text(lbl_status, "#FFDD00 初始化SD卡失败!#");
		goto out;
	}
	...
	{
		lv_label_set_text(lbl_status, "#FFDD00 警告:# 未找到有效的GPT分区!");
...
check_changes:
	...
	else if (has_mbr_attributes || gpt_emummc_migrate_no || gpt_oob_empty_part_no)
	{
		...
		if (gpt_emummc_migrate_no)
			s_printf(txt_buf + strlen(txt_buf), "- 虚拟系统 GPT分区地址和大小\n");
...
lv_res_t create_window_partition_manager(bool emmc)
{
	...
	if (!emmc)
	{
		win = nyx_create_standard_window(SYMBOL_SD" SD分区管理", NULL);
		lv_win_add_btn(win, NULL, SYMBOL_MODULES_ALT" 修复Hybrid MBR/GPT", _action_fix_mbr_gpt);
	}
	else
		win = nyx_create_standard_window(SYMBOL_CHIP" eMMC分区管理", NULL);
	...
	if (!emmc)
	{
		if (sd_mount())
		{
			...
			lv_label_set_text(lbl, "#FFDD00 初始化SD卡失败!#");
			return LV_RES_OK;
		}
	...
	else
	{
		if (emmc_initialize(false))
		{
			lv_obj_t *lbl = lv_label_create(h1, NULL);
			lv_label_set_recolor(lbl, true);
			lv_label_set_text(lbl, "#FFDD00 初始化eMMC失败!#");
			return LV_RES_OK;
		}
		emmc_set_partition(EMMC_GPP);
	}
	...
	if (!emmc)
	{
		...
		lv_label_set_static_text(lbl_emu, "#FF3C28 "SYMBOL_DOT" 虚拟系统 (RAW):#");
	...
	if (!emmc)
	{
		...
		lv_label_set_static_text(label_btn, SYMBOL_USB"  SD USB大容量存储");
	...
	lv_label_set_static_text(label_btn, SYMBOL_DOWNLOAD"  刷入Linux");
	if (!emmc)
	...
	case 0: // Disable Flash Android button if partition not found.
		lv_label_set_static_text(label_btn, SYMBOL_DOWNLOAD"  刷入Android");
		lv_obj_set_click(btn_flash_android, false);
		lv_btn_set_state(btn_flash_android, LV_BTN_STATE_INA);
		break;
	case 1: // Android 11.
		lv_label_set_static_text(label_btn, SYMBOL_DOWNLOAD"  刷入Android 11");
		break;
	case 2: // Android 13+
		lv_label_set_static_text(label_btn, SYMBOL_DOWNLOAD"  刷入Android 13+");
		break;
	}
	...
	lv_btn_set_fit(btn1, true, true);
	lv_label_set_static_text(label_btn, SYMBOL_SD"  下一步");
